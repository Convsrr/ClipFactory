...
- This repository includes a self-hosted provider in `tracker/`. Deploy it as a second Railway service with `Dockerfile.tracker`, copy the worker's read-only R2 variables to it, and set a generated `TRACKER_API_KEY`. Point the worker's `FACE_TRACKER_URL` at the tracker `/track` endpoint and use the same secret as `FACE_TRACKER_API_KEY`. The tracker removes global camera motion before selecting a confidence-gated action region; uncertain results deliberately return no track so sports rendering retains the full frame.
